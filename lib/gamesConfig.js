const generateNumbers = (_n, initAtZero = false) => [...Array(_n)
  .keys()].map(it => it + (initAtZero ? 0 : 1));
...
export const GetAvailableGames = () => Object.keys(GamesConfig).map(it => {
  return { slug: it, name: translate[it] };
});
